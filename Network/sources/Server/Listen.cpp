    ...
    while (tcp_server_.opened_) {
        try {
            ...
            if (!CheckMagic(packet.header_.magic)) {
                std::cout << "HUH: " << packet.header_.magic[0] << "," << packet.header_.magic[1] << "," << packet.header_.magic[2] << "," << std::endl;
                continue;
            }
            ...
            const auto it = std::ranges::find_if(clients_.begin(), clients_.end(), [client](const ClientStruct& c1) {
...
void Server::AcceptorRoutine()
{
    while (tcp_server_.opened_) {
        try {
            ...
            if (verbose_)
                ABN_DEBUG(SERVER_DEBUG, "[ACCEPTOR-ROUTINE]" << ABN_EXCEPTION, e.what())
        }
    }
}
